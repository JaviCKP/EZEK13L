...
$reportPath = Join-Path $repoRoot "VALIDACION_POC.md"
...
$cacheDir = Join-Path $repoRoot "data\.baseline_cache"
...
# ---------------------------------------------------------------------------
# Normal baseline caching — process the normal PCAP once, reuse everywhere
# ---------------------------------------------------------------------------
...
function Build-NormalBaseline {
  ...
  # Process through the full pipeline (suricata → zeek → features → score)
...
function Restore-NormalBaseline {
  # Restore the cached normal baseline results into live state.
  # This avoids re-running suricata+zeek+features+score for the same
  # normal PCAP on every scenario.
  Reset-LiveState
...
function Run-Control {
  # For control, just restore the baseline — no attack to process
...
function Run-Attack {
  ...
  # Restore the cached normal baseline
  Restore-NormalBaseline
  ...
  # Generate and process the attack PCAP only
  Invoke-DockerExec "lab-ndr-py-persistent" @(
    "python", "simulation/inject_attack.py",
...
# --- Start persistent containers (pay the startup cost once) ---
Start-PersistentContainers

try {
  # --- Build the normal baseline once and cache it ---
  Build-NormalBaseline

  $results = @()
  $results += @(Run-Control | Where-Object { $_.PSObject.Properties.Name -contains "Passed" })
  $results += @(Run-Attack -Choice "1" -Scenario "Port Scan" -ExpectedFactor "scan_ports_60s" -ExpectedSignaturePattern "SYN scan" -ExpectedClass "port_scan" | Where-Object { $_.PSObject.Properties.Name -contains "Passed" })
  $results += @(Run-Attack -Choice "2" -Scenario "DNS Exfiltration" -ExpectedFactor "dns_query_shape" -ExpectedSignaturePattern "" -ExpectedClass "dns_exfiltration" | Where-Object { $_.PSObject.Properties.Name -contains "Passed" })
  $results += @(Run-Attack -Choice "3" -Scenario "Brute Force HTTP" -ExpectedFactor "" -ExpectedSignaturePattern "login POST burst" -ExpectedClass "brute_force_http" | Where-Object { $_.PSObject.Properties.Name -contains "Passed" })
  $results += @(Run-Attack -Choice "4" -Scenario "SQL Injection" -ExpectedFactor "http_payload_shape" -ExpectedSignaturePattern "SQL injection" -ExpectedClass "sql_injection" | Where-Object { $_.PSObject.Properties.Name -contains "Passed" })
  $results += @(Run-Attack -Choice "5" -Scenario "Data Exfiltration" -ExpectedFactor "volume_spike" -ExpectedSignaturePattern "" -ExpectedClass "data_exfiltration" | Where-Object { $_.PSObject.Properties.Name -contains "Passed" })
} finally {
  # --- Always tear down persistent containers ---
...
$allPassed = @($results | Where-Object { $_.Passed -ne "PASS" }).Count -eq 0
$lines += ""
$lines += if ($allPassed) { "Conclusion: la matriz minima de la PoC pasa completa." } else { "Conclusion: hay fallos en la matriz minima de la PoC." }

Set-Content -LiteralPath $reportPath -Value $lines -Encoding UTF8

# Clean the baseline cache
